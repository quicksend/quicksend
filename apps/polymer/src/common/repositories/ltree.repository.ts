...
export class LTreeRepository<Entity extends LTreeEntity> extends EntityRepository<Entity> {
  async findClosestAncestor(path: string, filter?: QBFilterQuery<Entity>): Promise<Entity | null> {
    ...
    return queryBuilder
      .orderBy({ ["nlevel(path)"]: "DESC" }) // Get the parent that is closest to the path
      .limit(1)
      .getSingleResult();
  }
  ...
  async isDescendant(parent: Entity, descendant: Entity): Promise<boolean> {
    const node = await this.createQueryBuilder()
      .select("*")
      .where("path = ?", [parent.path])
      .andWhere("path <@ ?", [descendant.path])
      .getSingleResult();
